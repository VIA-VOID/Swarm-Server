...
public:
	...
	{
		// 자동화 코드
		_handlers[SC_PLAYER] = [](Session* session, BYTE* buffer, int32 len)
			{
				ServerPacketHandler::HandlePacket<Protocol::SC_PLAYER>(Handle_SC_PLAYER, session, buffer, len);
			};
	}
	...
	// 자동화 코드
	static void Handle_SC_PLAYER(Session* session, Protocol::SC_PLAYER& packet);
...
// 전달받은 RunFunc 함수 실행
template<typename PacketType, typename RunFunc>
inline void ServerPacketHandler::HandlePacket(RunFunc func, Session* session, BYTE* buffer, uint16 len)
{
	PacketType packet;
...
// 패킷 전송
template<typename T>
inline void ServerPacketHandler::SendPacket(Session* session, const T& packet, uint16 packetId)
{
